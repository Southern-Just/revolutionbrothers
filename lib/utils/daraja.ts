...
export async function stkPush(input: {
  ...
  try {
    ...
    // Check M-Pesa's response for success
    if (res.data.ResponseCode !== "0") {
      throw new Error(`STK_PUSH_FAILED: ${res.data.ResponseDescription || "Unknown error"}`);
    }

    // Update using transactionCode (which matches input.reference)
    await db
      .update(transactions)
      .set({ checkoutRequestId: res.data.CheckoutRequestID })
      .where(eq(transactions.transactionCode, input.reference));
